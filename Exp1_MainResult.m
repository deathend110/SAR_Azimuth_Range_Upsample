...
function plot_main_result_curves(Q_list, group_defs, psnr_mean, psnr_std, ...
    ...
    for metric_idx = 1:2
        ...
        % 1) NoUpsample 水平虚线
        y_noup = double(noupsample_val) * ones(1, numel(Q_list));
        h_noup = plot(Q_list, y_noup, ...
        ...
        h_bi_scat = plot(Q_list, best_bi_vals, "s", ...
            ...
            "DisplayName", "best bi-dir");
        h_bi_line = plot(Q_list, best_bi_vals, "-", ...
            "Color", colors.balanced, "LineWidth", 1.8, ...
            "HandleVisibility", "off");

        xlabel("Total Upsample ratio Q", "FontSize", 13);
        ylabel(y_label, "FontSize", 13);
        % title(sprintf("主结果：四类对照 %s vs Q (As=%.1f, 7数据集×10帧)", tag, 0.6), "FontSize", 14);
