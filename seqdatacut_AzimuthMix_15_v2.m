...
%% =========================================================
%  主脚本：生成方位向 mixed 采样率序列数据集（15帧，去高斯噪声净化版）
%  当前脚本对应 q=3，在方位向RC结果上按掩码拼接 q倍上采样链路和60MHz链路。
%
%  核心逻辑：
%  1) 全局512有效轴为 H(768)-L(768)-H(768)，对应帧模式：3高-3混-3低-3混-3高。
%  2) 每条序列先切 1200x2992 的 60MHz 大块，再逐帧取 1200x1200 patch。
%  3) 序列级生成同一个 master 随机相位RT阈值场，保证15帧阈值相位连续。
%  4) high帧走方位向 q 倍上采样RT 1-bit链路；low帧走60MHz RT 1-bit链路。
...
q             = 7;       % 方位向上采样倍率
As            = 0.6;     % RT 阈值系数
EDGE_BUFFER   = 600;      % mixed 能量对齐 buffer

% 数据集命名
BASE_TAG = "Sequence_Dataset_AzimuthMix_q" + string(q) + "_V2";
...
for d = 1:length(DIR_LIST)
    ...
    % 载入该底图归一化参数文件
    save_path = ROOT_DIR + "_azimuthq"+ num2str(q) + ".mat";
        ...
        for i = 1:length(current_files)
            ...
            for block_start = block_starts
                ...
                for k = 1:N_SEQ
                    ...
                    % imagesc(img_gt_l_600);axis image;colorbar;
                    ...
                    % ---------- input 成像 ----------
                     switch string(frame_mode)
                        case "low"
                            ...
                            % imagesc(norm_L_low);axis image;colorbar;
                            seq_input_L(:,:,k) = crop_center(norm_L_low,  PATCH_SIZE);

                        case "high"
                            % 上采样
                            signal60_patch_high = azimuth_upsample_fft(signal60_input_patch, q);
                            % RT阈值1bit量化
                            ...
                            % imagesc(norm_L_high);axis image;colorbar;
                        ...
                        case "mixed"
                            ...
                            RC_high = Range_Compress(channel_1bit_high, S60.fc, S60.tnrn, S60.gama, S60.R0, S60.C, S60.Fs, S60.Tp);
                            % 下采样,得上采样RC
                            RC_up = crop_azimuth_doppler_to_width(RC_high, S60.nan);
                            ...
                            % 拼接q3和q1的RC
                            % 能量对齐
                            [RC_mix, ~] = energy_crop(RC_up, RC_down, mode_mask_1200, EDGE_BUFFER, q);
                            ...
                            % imagesc(norm_L_mix);axis image;colorbar;
...
function [U_master_seq, sigma_seq, A_rt] = build_master_threshold_seq(seq60_input, q, As)
    seq_up = azimuth_upsample_fft(seq60_input, q);   % [1200 x (q*2992)]
