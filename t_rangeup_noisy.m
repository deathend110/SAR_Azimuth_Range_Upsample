...
%% Range Upsample noisy
% 生成高斯噪声
noisy = gaussian(signal60_input, SNR_dB);
signal60_input_noisy = signal60_input + noisy;

% 上采样
signal60_patch_high_noisy = range_upsample_fft(signal60_input_noisy, Range_q);

% 生成RT阈值
[U_master_patch, ~, ~] = Range_Build_RT(signal60_patch_high_noisy, As_rt);
...
Range_title = [
    "Range Upsample noisy q" + num2str(Range_q) + " 1bit"; 
    "SSIM: " + num2str(ssim(Range_Upsample_noisy, img_gt_noisy))+"   PSNR: " + num2str(psnr(Range_Upsample_noisy, img_gt_noisy))
    ];
subplot(224);imagesc(Range_Upsample_noisy);axis image;colorbar;title(Range_title);
