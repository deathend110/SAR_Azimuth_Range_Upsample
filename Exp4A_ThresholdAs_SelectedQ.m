...
%% =========================================================
%  实验4A：固定Q=4/R2A2下的NCT与SplitRT阈值幅度曲线
%  输出：NCT/RT分别对应PSNR与SSIM四张带标准差误差线的图
%% =========================================================
...
cfg = struct("Q", 4, "Range_q", 2, "Azimuth_q", 2, "group_name", "R2A2");
As_list = 0:0.1:1.5;
...
output_dir = fullfile(pwd, "Exp4A_ThresholdAs_SelectedQ_Output");
...
psnr_all_nct = zeros(num_As, total_samples);
ssim_all_nct = zeros(num_As, total_samples);
psnr_all_rt  = zeros(num_As, total_samples);
ssim_all_rt  = zeros(num_As, total_samples);

fprintf("=== Exp4A: Q=%d/%s, %d As values, %d samples ===\n", cfg.Q, cfg.group_name, num_As, total_samples);
for as_idx = 1:num_As
    ...
    for s_idx = 1:total_samples
        ...
        psnr_all_nct(as_idx, s_idx) = psnr(img_nct, img_gt);
        ssim_all_nct(as_idx, s_idx) = ssim(img_nct, img_gt);
        psnr_all_rt(as_idx, s_idx)  = psnr(img_rt, img_gt);
        ssim_all_rt(as_idx, s_idx)  = ssim(img_rt, img_gt);
    end
end

psnr_mean_nct = mean(psnr_all_nct, 2);
psnr_std_nct  = std(psnr_all_nct, 0, 2);
ssim_mean_nct = mean(ssim_all_nct, 2);
ssim_std_nct  = std(ssim_all_nct, 0, 2);
psnr_mean_rt  = mean(psnr_all_rt, 2);
psnr_std_rt   = std(psnr_all_rt, 0, 2);
ssim_mean_rt  = mean(ssim_all_rt, 2);
ssim_std_rt   = std(ssim_all_rt, 0, 2);

summary_table = table( ...
    repmat(cfg.Q, num_As * 2, 1), ...
    repmat(cfg.Range_q, num_As * 2, 1), ...
    repmat(cfg.Azimuth_q, num_As * 2, 1), ...
    repmat(string(cfg.group_name), num_As * 2, 1), ...
    [repmat("NCT", num_As, 1); repmat("RT", num_As, 1)], ...
    [As_list(:); As_list(:)], ...
    [psnr_mean_nct; psnr_mean_rt], ...
    [psnr_std_nct; psnr_std_rt], ...
    [ssim_mean_nct; ssim_mean_rt], ...
    [ssim_std_nct; ssim_std_rt], ...
    repmat(total_samples, num_As * 2, 1), ...
...
plot_single_threshold_curve(As_list, psnr_mean_nct, psnr_std_nct, "PSNR (dB)", "NCT", "PSNR", output_dir);
plot_single_threshold_curve(As_list, ssim_mean_nct, ssim_std_nct, "SSIM", "NCT", "SSIM", output_dir);
plot_single_threshold_curve(As_list, psnr_mean_rt, psnr_std_rt, "PSNR (dB)", "RT", "PSNR", output_dir);
...
function plot_single_threshold_curve(As_list, metric_mean, metric_std, y_label, threshold_name, metric_tag, output_dir)
    ...
    eb = errorbar(As_list, metric_mean, metric_std, "-o", ...
        "LineWidth", 1.8, "MarkerSize", 6, "CapSize", 8);
    [best_val, best_idx] = max(metric_mean);
    plot(As_list(best_idx), best_val, "p", "MarkerSize", 16, ...
        "MarkerFaceColor", "r", "MarkerEdgeColor", "r", "HandleVisibility", "off");
    xlabel("Threshold amplitude (× \sigma_{ref})", "FontSize", 13);
    ylabel(y_label, "FontSize", 13);
    title(sprintf("%s vs %s threshold amplitude", metric_tag, threshold_name), "FontSize", 14);
    legend(eb, sprintf("%s mean \\pm std (n=70)", threshold_name), "Location", "best");
    ax = gca;
    ax.FontSize = 12;
    save_name = sprintf("Exp4A_%s_%s_AsCurve.png", threshold_name, metric_tag);
