...
for iq = 1:numel(q_list)
    ...
    %% ---------- 1) 按 ZT.m 的方式做方位向上采样 ----------
    channel_up = azimuth_upsample_fft(channel_1, q);   % nrn x (q*nan)

    nan_up = size(channel_up, 2);
    prf_up = q * prf;
    fnan_up = ((-nan_up/2 : nan_up/2-1).' * prf_up / nan_up);

    %% ---------- 2) ZT ----------
    channel_zt_1bit = quantize_1bit_zero(channel_up);

    RC_zt_up   = Range_Compress(channel_zt_1bit, fc, tnrn, gama, R0, C, Fs, Tp);
    RCMC_zt_up = RCMC(RC_zt_up, lambda, fnrn, fnan_up, R0, C, v);
    RCMC_zt_crop = crop_azimuth_doppler_to_width(RCMC_zt_up, nan);
    ...
    for iA = 1:numel(A_list_nct)
        ...
        RC_nct_up   = Range_Compress(channel_nct_1bit, fc, tnrn, gama, R0, C, Fs, Tp);
        RCMC_nct_up = RCMC(RC_nct_up, lambda, fnrn, fnan_up, R0, C, v);
        RCMC_nct_crop = crop_azimuth_doppler_to_width(RCMC_nct_up, nan);
    ...
    for iK = 1:numel(A_list_rt)
        ...
        for r = 1:n_repeat_rt
            ...
            RC_rt_up   = Range_Compress(channel_rt_1bit, fc, tnrn, gama, R0, C, Fs, Tp);
            RCMC_rt_up = RCMC(RC_rt_up, lambda, fnrn, fnan_up, R0, C, v);
            RCMC_rt_crop = crop_azimuth_doppler_to_width(RCMC_rt_up, nan);
...
ylabel('SSIM');
ylim([0.5 1.0]);
title('SSIM vs q for ZT / NCT / RT (azimuth upsampling + Doppler crop)');
...
xlabel('q');
ylabel('PSNR (dB)');
title('PSNR vs q for ZT / NCT / RT (azimuth upsampling + Doppler crop)');
legend('ZT','NCT(best per q)','RT(best per q)','Location','best');
