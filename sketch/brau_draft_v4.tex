...
The main allocation experiment uses a separable random threshold (RT). Its phase is the sum of independent range and azimuth phase vectors:
\begin{equation}
    u_{\mathrm{RT}}(m,n)
...
The range single-frequency threshold (RSFT) is used to study the joint influence of threshold strength and frequency:
\begin{equation}
    u_{\mathrm{RSFT}}(\tau)
...
Reconstruction fidelity is evaluated using peak signal-to-noise ratio (PSNR) and structural similarity (SSIM) \cite{wang2004ssim}. Image entropy is computed from the normalized amplitude image using a fixed 256-bin histogram:
\begin{equation}
    H=-\sum_{b=1}^{256}p_b\log_2 p_b.
\end{equation}
Equivalent number of looks (ENL) is evaluated on intensity, obtained by squaring the normalized amplitude:
\begin{equation}
...
For each budget, the best bidirectional and unidirectional groups are selected by mean PSNR. Paired Wilcoxon signed-rank tests are applied to the corresponding 70-sample vectors.

\section{Experiments and Results}

\subsection{Mechanism Analysis Through Spectral Leakage}

The working interpretation of BRAU is that the effective SAR echo occupies a two-dimensional range-azimuth spectral support, whereas one-bit quantization produces distortion in both dimensions. A unidirectional allocation enlarges only one spectral axis. In contrast, a bidirectional allocation supplies frequency-domain redundancy in both dimensions and can reduce the overlap between quantization distortion and the effective support. Fig.~\ref{fig:mechanism} compares representative allocations for $Q_{\mathrm{up}}=4$ on the common frequency range $[-2,2]\times[-2,2]$.
...
The threshold parameters influence absolute reconstruction quality and therefore require separate calibration. Fig.~\ref{fig:threshold_design}(a) shows the RT amplitude sensitivity for the best bidirectional allocations at $Q_{\mathrm{up}}=4,6,9$. The SSIM first increases and then decreases with $A_s$. The best tested values shift from $A_s=0.7$ for $Q_{\mathrm{up}}=4$ to $0.8$ for $Q_{\mathrm{up}}=6$ and $1.0$ for $Q_{\mathrm{up}}=9$. The common $A_s=0.6$ used in the main experiment is thus a controlled setting for allocation comparison rather than a global optimum for every budget.
