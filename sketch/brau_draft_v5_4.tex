...
SQ-chen@hotmail.com}
\and
\IEEEauthorblockN{4\textsuperscript{th} Yuan Chen}
...
chenyuan15@ustb.edu.cn}
\and
\IEEEauthorblockN{5\textsuperscript{th} Pui-Lam Ng}
...
\begin{abstract}
One-bit acquisition reduces the raw-data burden of high-resolution synthetic aperture radar (SAR), but how a fixed pre-quantization upsampling budget should be allocated between range and azimuth remains unclear. We propose bidirectional range-azimuth upsampling (BRAU), which distributes this budget across both echo dimensions rather than concentrating it on one axis. Under a common separable random threshold and five integer budgets, bidirectional allocations consistently outperform unidirectional ones and form a near-optimal plateau. Range-compressed spectra further show reduced off-support and directional leakage, supporting the role of two-dimensional spectral redundancy.
\end{abstract}
...
More recent work emphasizes acquisition thresholds and quantization-distortion suppression. Time-varying thresholds retain amplitude-related information \cite{demir2018onebit}. Single-frequency thresholds relocate dominant harmonics \cite{zhao2019onebit}. Low-precision sampling strategies improve SAR image quality \cite{zhao2021lowprecision}. Two-bit acquisition provides a related rate-quality tradeoff \cite{liu2023twobit}. Learned de-quantization suppresses harmonics after one-bit acquisition \cite{si2023dequantization}. Fixed thresholds with slow-time fluctuations offer another acquisition-side design \cite{nie2025fixed}. These studies optimize a threshold, observation model, or reconstruction method, but do not isolate how a fixed upsampling budget should be divided between range and azimuth.

Because range and azimuth contribute differently to SAR focusing, equal upsampling budgets may yield different one-bit observations. We therefore study the allocation of a fixed pre-quantization budget $Q_{\mathrm{up}}=RA$ between the range factor $R$ and azimuth factor $A$, and propose bidirectional range-azimuth upsampling (BRAU) to distribute the budget across both dimensions rather than one axis. Across five budgets, bidirectional allocations consistently outperform their unidirectional counterparts, while neighboring factor pairs form a near-optimal plateau. Range-compressed spectra further show reduced off-support and directional leakage, supporting the benefit of two-dimensional spectral redundancy.
...
\end{equation}
Although the spectral embedding is expressed in two dimensions, the implementation exploits separability through successive one-dimensional FFT-based interpolation along range and azimuth. For a fixed $Q_{\mathrm{up}}=RA$, every allocation reaches the same enlarged-grid sample count $N_r^{\uparrow}N_a^{\uparrow}=Q_{\mathrm{up}}N_rN_a$, while practical runtime varies with FFT lengths and intermediate matrix shapes.
...
The complete BRAU processing sequence is summarized in Table~\ref{tab:brau_pipeline}.
...
Phase-quantized SAR processing produces nonlinear harmonic distortion \cite{franceschetti1999phase}. Single-frequency thresholds can relocate dominant harmonics \cite{zhao2019onebit}, while learned de-quantization suppresses harmonics in reconstructed one-bit SAR images \cite{si2023dequantization}. For the two-dimensional echo considered here, the effective spectral support spans range and azimuth. Under a fixed $Q_{\mathrm{up}}$, a unidirectional allocation leaves one spectral axis unexpanded, whereas $R>1$ and $A>1$ distribute redundancy across both axes and can reduce the overlap between quantization distortion and the effective support. This provides a structural explanation for the observed leakage reduction without implying a unique globally optimal factor pair.
...
PSNR and SSIM assess all paired samples \cite{wang2004ssim}. ENL, computed from squared normalized amplitude as $\mathrm{ENL}=\mu_I^2/\sigma_I^2$ \cite{feng2015tgv}, uses 20 samples from the field and port datasets with one fixed $64\times64$ GT-selected ROI per sample. For each budget, the best bidirectional and unidirectional allocations are selected by mean PSNR and compared using paired Wilcoxon signed-rank tests \cite{wilcoxon1945ranking}.

The mechanism experiment compares R1A1, R4A1, R1A4, and R2A2 at $Q_{\mathrm{up}}=4$ over a common $[-2,2]\times[-2,2]$ normalized-frequency window with support threshold $\tau=0.35$; threshold sensitivity sweeps $A_s$ for the best bidirectional allocations at $Q_{\mathrm{up}}=4,6,9$. For each sample and SNR, 50 complex Gaussian realizations with different seeds are shared across R4A1, R1A4, and R2A2 at $Q_{\mathrm{up}}=4$ and $A_s=0.6$, with SNRs of $-2$, 2.5, 7, and 11.5~dB plus a noise-free baseline.
...
Table~\ref{tab:evaluation_indexes} shows that a bidirectional allocation attains the highest PSNR and SSIM at every budget. Against unidirectional allocations, the gains are 0.2856--0.5072~dB in PSNR and 0.0169--0.0221 in SSIM ($p_{\mathrm{PSNR}}\leq4.34\times10^{-10}$ and $p_{\mathrm{SSIM}}\leq3.56\times10^{-13}$). Neighboring bidirectional factor pairs achieve similar performance, supporting a near-optimal region.

The PSNR-selected bidirectional allocations required 550.6--1141.8~ms per sample, remaining close to the azimuth-only runtime while incurring 5.6--15.0\% (10.8\% on average) overhead relative to the faster range-only allocation under the same budget.
