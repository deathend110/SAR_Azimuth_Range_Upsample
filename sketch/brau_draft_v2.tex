...
The main experiment uses seed 2026, SplitRT with $A_s=0.6$, and seven SAR datasets: Bangkok, city1, city2, SAR-figure, field, port, and suburb scenes. The datasets are derived from the Capella Open Data IEEE Data Contest 2026 collection.\footnote{\url{https://radiantearth.github.io/stac-browser/#/external/capella-open-data.s3.us-west-2.amazonaws.com/stac/capella-open-data-ieee-data-contest/collection.json}} Ten stratified windows are selected from each dataset, giving 70 paired test samples. The ground-truth image is formed from the unquantized raw echo using the same SAR chain: range compression, RCMC, image formation, ROI extraction, and image normalization. Each one-bit result is compared with this normalized ground truth using peak signal-to-noise ratio (PSNR) and structural similarity (SSIM).
