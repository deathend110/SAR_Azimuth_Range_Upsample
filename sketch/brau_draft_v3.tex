...
We next test whether the bidirectional advantage depends on a specific threshold construction. First, SplitRT and FullRT are compared over $A_s \in [0,1]$ for representative budgets. Their PSNR and SSIM curves nearly overlap, with only small differences across the tested range. We then examine the threshold-amplitude sensitivity of NCT and SplitRT for the best bidirectional allocations under $Q_{\mathrm{up}}=4,6,9$, as shown in Fig.~\ref{fig:threshold_as_byq}. The NCT curves peak at a small threshold amplitude and then degrade rapidly as $A_s$ increases, whereas SplitRT reaches its best SSIM at a moderate amplitude whose value increases with the upsampling budget. This shows that the threshold amplitude affects absolute reconstruction quality, but it does not constitute the source of the bidirectional allocation effect.
