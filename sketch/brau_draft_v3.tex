...
\begin{abstract}
One-bit synthetic aperture radar (SAR) imaging reduces data precision to sign information, making image quality sensitive to quantization distortion. This paper studies whether a fixed pre-quantization digital upsampling budget should be concentrated in one SAR dimension or split across range and azimuth. We formulate bidirectional range-azimuth upsampling (BRAU), where the total allocation $Q_{\mathrm{up}}=R\times A$ is fixed and FFT zero-padding is applied to the raw echo before threshold generation and one-bit quantization. BRAU is therefore a frequency-domain raw-data operation rather than post-reconstruction image interpolation. Experiments on seven SAR datasets with 70 paired samples show that the best bidirectional allocations consistently outperform the best unidirectional allocations across all tested budgets, improving PSNR by 0.2856--0.5072 dB and SSIM by 0.0169--0.0221. The advantage remains under zero-threshold, non-random constant-threshold, and random-threshold quantization. Range-compressed spectral analysis further shows lower off-support and directional leakage, supporting bidirectional allocation as an effective strategy for one-bit SAR imaging.
\end{abstract}
...
This paper presents bidirectional range-azimuth upsampling (BRAU) as a fixed-budget allocation strategy for one-bit SAR imaging. BRAU performs zero-padding upsampling on the raw echo grid before threshold generation and one-bit quantization, rather than interpolating a reconstructed image. The engineering meaning is not only sample-grid enlargement: BRAU probes how two-dimensional oversampling changes the spectral distribution of one-bit quantization distortion relative to the effective range-azimuth SAR support. Experiments on seven SAR datasets show that, for all tested budgets, the best bidirectional allocation consistently outperforms the best unidirectional allocation in both PSNR and SSIM. The advantage remains significant under zero-threshold, non-random constant-threshold, and random-threshold settings. Mechanism analysis further shows that bidirectional allocation produces lower off-support and directional spectral leakage after range compression, supporting the interpretation that range-azimuth allocation better matches the two-dimensional spectral structure of SAR echoes.
...
\begin{itemize}
    ...
    \item We verify that the advantage is robust to different threshold constructions.
...
We evaluate three threshold settings. Zero threshold (ZT) directly applies sign quantization with $u=0$. Non-random constant threshold (NCT) uses a spatially uniform deterministic threshold instead of a sample-varying random field. In our implementation, the threshold amplitude is normalized by the upsampled-echo scale $\hat{\sigma}=\sqrt{2/\pi}\cdot\operatorname{mean}(|s_{\uparrow}|)$, so the amplitude-sensitivity experiments use $u_{\mathrm{NCT}}=A_s\hat{\sigma}$ with a fixed phase. The same threshold is added to all samples before applying the one-bit sign operation separately to the in-phase and quadrature channels. Random threshold (RT) uses a random-phase threshold with fixed amplitude $|U|=A_s\hat{\sigma}$, where $\hat{\sigma}= \sqrt{2/\pi}\cdot\operatorname{mean}(|s(\tau,\eta)|)$ estimates the signal RMS amplitude and $A_s$ is a dimensionless scaling factor controlling the threshold amplitude relative to the signal level. For RT, we compare two phase construction variants: SplitRT generates the threshold phase as the sum of independent range-phase and azimuth-phase vectors (separable, $N_r+N_a$ degrees of freedom), while FullRT assigns an independent random phase to each sample (non-separable, $N_r\times N_a$ degrees of freedom). This comparison tests whether the spatial structure of the threshold phase affects the allocation result.
...
The main experiment uses seed 2026, SplitRT with $A_s=0.6$, and SAR scenes across various landforms (including urban areas, farmlands, suburbs, and sea surfaces) to ensure data diversity. The datasets are derived from the Capella Open Data IEEE Data Contest 2026 collection.\footnote{\href{https://radiantearth.github.io/stac-browser/\#/external/capella-open-data.s3.us-west-2.amazonaws.com/stac/capella-open-data-ieee-data-contest/collection.json}{Capella Open Data IEEE Data Contest 2026 collection}} Ten stratified windows are selected from each dataset, giving 70 paired test samples. The ground-truth image is formed from the unquantized raw echo using the same SAR chain: range compression, RCMC, image formation, ROI extraction, and image normalization. Each one-bit result is compared with this normalized ground truth using peak signal-to-noise ratio (PSNR) and structural similarity (SSIM).
...
\begin{figure}[t]
\centerline{\includegraphics[width=\linewidth]{../assert/Exp2_Node2_RC_Spectra.png}}
...
For an intermediate matrix $X$, let its two-dimensional spectral energy be
\begin{equation}
...
Table~\ref{tab:mechanism_metrics} quantifies the visual trend in Fig.~\ref{fig:mechanism}. After RC and frequency-domain crop, R2A2 achieves the lowest off-support ratio, range leakage ratio, and azimuth leakage ratio among all upsampled groups. Since RCMC is a phase-correction step in the range-Doppler domain, it does not materially change the spectral-energy support measured by these leakage metrics; therefore, Table~\ref{tab:mechanism_metrics} reports the RC-stage metrics. These off-support and directional leakage results support the interpretation that bidirectional oversampling helps move one-bit quantization harmonics and distortion away from the effective two-dimensional SAR spectral support. This evidence links the intermediate spectra to the final metrics: Together with the main PSNR and SSIM gains reported in the final subsection, this representative mechanism case shows that the bidirectional group also has the least spectral leakage at the intermediate RC stage.

\begin{table*}[t]
\caption{Best bidirectional and unidirectional results under each fixed budget. Best groups are selected by mean PSNR; values are mean $\pm$ standard deviation over 70 paired samples.}
\begin{center}
\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{tabular}{c c c c c c c c c}
\hline
$Q_{\mathrm{up}}$ & Best bi-dir. & Best uni-dir. & Bi-dir. PSNR & Uni-dir. PSNR & Bi-dir. SSIM & Uni-dir. SSIM & $\Delta$PSNR & $\Delta$SSIM \\
\hline
4  & R2A2 & R4A1  & 26.0623$\pm$1.4358 & 25.7767$\pm$1.4104 & 0.8026$\pm$0.0263 & 0.7857$\pm$0.0277 & 0.2856 & 0.0169 \\
6  & R2A3 & R1A6  & 26.5585$\pm$1.4797 & 26.1415$\pm$1.4268 & 0.8246$\pm$0.0248 & 0.8037$\pm$0.0249 & 0.4170 & 0.0208 \\
8  & R2A4 & R8A1  & 26.8194$\pm$1.5110 & 26.3618$\pm$1.4619 & 0.8363$\pm$0.0215 & 0.8148$\pm$0.0249 & 0.4576 & 0.0215 \\
9  & R3A3 & R9A1  & 26.9167$\pm$1.5381 & 26.4482$\pm$1.4724 & 0.8400$\pm$0.0228 & 0.8186$\pm$0.0233 & 0.4685 & 0.0214 \\
10 & R5A2 & R10A1 & 26.9702$\pm$1.5220 & 26.4630$\pm$1.4602 & 0.8427$\pm$0.0212 & 0.8206$\pm$0.0235 & 0.5072 & 0.0221 \\
\hline
\end{tabular}
\label{tab:main_gain}
\end{center}
...
We next test whether the bidirectional advantage depends on a particular threshold-amplitude design. Fig.~\ref{fig:threshold_as_byq} reports the SSIM sensitivity of NCT and SplitRT for the best bidirectional allocations under $Q_{\mathrm{up}}=4,6,9$. For NCT, the best SSIM is obtained at a small threshold amplitude ($A_s=0.1$ for all three budgets), and the curves degrade as $A_s$ increases. For SplitRT, the optimal threshold amplitude is larger and shifts with the upsampling budget, with the best $A_s$ changing from 0.7 to 0.8 and 1.0 for $Q_{\mathrm{up}}=4,6,9$, respectively. These trends show that threshold amplitude changes the absolute reconstruction quality, but the following threshold-setting comparison confirms that the bidirectional allocation gain is not caused by a single tuned value of $A_s$.
...
\begin{tabular}{@{}cc@{}}
\includegraphics[width=0.48\linewidth]{../assert/NCT_SSIM_bestAs_curve.png} &
\includegraphics[width=0.48\linewidth]{../assert/RT_SSIM_bestAs_curve.png} \\
(a) NCT & (b) SplitRT
\end{tabular}
\caption{Threshold-amplitude sensitivity of bidirectional allocations under different upsampling budgets. The NCT and SplitRT SSIM curves are plotted against $A_s$ for the best bidirectional allocations at $Q_{\mathrm{up}}=4,6,9$. Markers indicate the best $A_s$ for each budget, and error bars denote one standard deviation over 70 paired samples.}
\label{fig:threshold_as_byq}
\end{figure}

Finally, we compare ZT, NCT, and RT as threshold settings rather than as tuned amplitude curves. In this robustness experiment, the unidirectional reference is computed as the per-sample maximum over range-only and azimuth-only groups, which is a stricter baseline than selecting one mean-best group. For $Q_{\mathrm{up}}=4,6,9$, the bidirectional group remains better than this unidirectional reference under all three threshold settings. Under ZT, the PSNR gains are 0.3612 dB, 0.4162 dB, and 0.4318 dB. Under NCT, the gains increase to 0.4308 dB, 0.4896 dB, and 0.5341 dB. Under RT, the gains remain positive at 0.2018 dB, 0.3306 dB, and 0.3611 dB. Therefore, RT improves the absolute reconstruction quality, but it is not the source of the bidirectional allocation effect.

\subsection{Main Fixed-Budget Results}

\begin{figure}[!t]
\centerline{\includegraphics[width=\linewidth]{../assert/2D SplitRT As0.6.png}}
\caption{Two-dimensional SAR scene examples used in the SplitRT setting with $A_s=0.6$.}
\label{fig:intro_scene_examples}
\end{figure}
Fig.~\ref{fig:intro_scene_examples} shows representative $Q_{\mathrm{up}}=4$ SplitRT examples, including the ground truth, range-only, azimuth-only, and bidirectional R2A2 results. We then evaluate whether splitting a fixed upsampling budget across range and azimuth improves reconstruction quality compared with concentrating the same budget in a single dimension. We test budgets $Q_{\mathrm{up}}=4,6,8,9,10$. For each $Q_{\mathrm{up}}$, all available factor pairs are grouped into no-upsampling, range-only, azimuth-only, and bidirectional allocations.
...
The no-upsampling baseline gives the lowest reconstruction quality. Increasing the total budget improves all upsampled groups, but the bidirectional groups consistently outperform the range-only and azimuth-only baselines. This result supports the central claim that the dimensional allocation of the upsampling budget matters in one-bit SAR imaging.

\section{Conclusion}

This paper investigated fixed-budget frequency-domain range-azimuth upsampling allocation for one-bit SAR imaging. Instead of asking only how to design the quantization threshold or how to recover an image after one-bit acquisition, we studied how a fixed pre-quantization digital budget $Q_{\mathrm{up}}=R\times A$ should be distributed between the range and azimuth dimensions. The experimental results show that bidirectional allocation consistently outperforms the best unidirectional allocation across all tested budgets.
