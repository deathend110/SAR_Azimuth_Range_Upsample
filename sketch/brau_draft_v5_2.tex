...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
Before upsampling and one-bit quantization, three seeded Gaussian draws per sample are power-scaled across SNRs, with each resulting noisy echo shared by all allocations. R4A1, R1A4, and R2A2 are evaluated at $Q_{\mathrm{up}}=4$ and $A_s=0.6$ over 70 samples; Table~\ref{tab:noise_ablation} reports four representative finite SNRs and the no-added-noise baseline.
...
{\renewcommand{\footnotesize}{\fontsize{7.2pt}{8.1pt}\selectfont}
