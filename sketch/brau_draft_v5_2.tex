...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
The experiment compares R4A1, R1A4, and R2A2 at $Q_{\mathrm{up}}=4$ with $A_s=0.6$ over the same 70 samples. Noise is added before upsampling and one-bit quantization, with all allocations sharing the same noisy echo for each sample and SNR. Although both clean-GT and same-noise-GT protocols are computed, Table~\ref{tab:noise_ablation} reports the same-noise-GT results at four representative finite SNRs and the no-added-noise baseline.

Under same-noise-GT evaluation, R2A2 achieves the highest PSNR and SSIM under every condition reported in Table~\ref{tab:noise_ablation}, retaining the advantage of bidirectional allocation in the presence of additive complex Gaussian noise. Under clean-GT evaluation, R2A2 remains the best allocation, with its PSNR/SSIM increasing from 20.4013~dB/0.4733 at $-2$~dB SNR to 25.7322~dB/0.7815 at 12~dB.
