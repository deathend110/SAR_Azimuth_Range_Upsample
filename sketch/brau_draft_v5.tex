...
\vspace{0pt}
\captionof{table}{Same-noise-GT reconstruction quality under additive complex Gaussian noise. The best PSNR and SSIM within each SNR block are bold.}
...
\hline
2.5 & R4A1 & 25.7515 & 0.8066 \\
    & R1A4 & 25.7534 & 0.8057 \\
    & R2A2 & \textbf{26.1825} & \textbf{0.8242} \\
\hline
7 & R4A1 & 25.9317 & 0.7998 \\
  & R1A4 & 25.9209 & 0.7985 \\
  & R2A2 & \textbf{26.3252} & \textbf{0.8170} \\
\hline
11.5 & R4A1 & 25.8546 & 0.7920 \\
     & R1A4 & 25.8153 & 0.7908 \\
     & R2A2 & \textbf{26.2268} & \textbf{0.8099} \\
\hline
...
\vspace{2pt}
\parbox{0.96\linewidth}{\footnotesize All values are averaged over 70 samples. Finite-SNR results average three noise realizations within each sample; $\infty$ denotes no added Gaussian noise and uses one run per sample.}
\end{minipage}\hfill
\begin{minipage}[t]{0.485\textwidth}
\vspace{0pt}
\centering
\includegraphics[width=\linewidth]{../assert/Noise_NoisyGT_FocusCurves.pdf}
\captionof{figure}{Same-noise-GT PSNR and SSIM over the complete added-noise SNR sweep with automatically scaled vertical axes. The $\infty$ markers denote no added Gaussian noise.}
\label{fig:noise_noisygt_focus}
\end{minipage}
\end{figure*}
...
\begin{equation}
    \begin{aligned}
        \widetilde{s} &= s+n,\qquad n\sim\mathcal{CN}(0,\sigma_n^2), \\
        \mathrm{SNR}_{n} &= 10\log_{10}\frac{\mathbb{E}|s|^2}{\mathbb{E}|n|^2}.
    \end{aligned}
\end{equation}
The experiment compares R4A1, R1A4, and R2A2 at $Q_{\mathrm{up}}=4$ with $A_s=0.6$ over the same 70 samples. Noise is added before upsampling and one-bit quantization, with all allocations sharing the same noisy echo for each sample and SNR. Although both clean-GT and same-noise-GT protocols are computed, Table~\ref{tab:noise_ablation} reports same-noise-GT results at four representative finite SNRs and the no-added-noise baseline, while Fig.~\ref{fig:noise_noisygt_focus} shows the complete SNR sweep.

Under same-noise-GT evaluation, R2A2 achieves the highest PSNR and SSIM under every condition reported in Table~\ref{tab:noise_ablation}, retaining the advantage of bidirectional allocation in the presence of additive complex Gaussian noise.
