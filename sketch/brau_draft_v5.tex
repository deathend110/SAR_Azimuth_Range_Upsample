...
\end{equation}
The experiment compares R4A1, R1A4, and R2A2 at $Q_{\mathrm{up}}=4$ with $A_s=0.6$ over the same 70 samples. For each SNR and sample, all allocations share the same noisy echo. Three noise realizations are averaged within each sample before dataset-level averaging. Reconstruction is evaluated against both the clean unquantized GT and the unquantized GT formed from the same noisy echo; the latter separates one-bit reconstruction fidelity from the degradation caused directly by the added noise. Table~\ref{tab:noise_ablation} reports four representative levels from the full $-2$ to 12~dB sweep with 0.5~dB spacing.

\begin{table}[t]
\caption{Noise robustness at selected input SNRs. The best PSNR and SSIM within each SNR block are bold.}
\label{tab:noise_ablation}
\centering
\scriptsize
\renewcommand{\arraystretch}{0.84}
\setlength{\tabcolsep}{5pt}
\textbf{(a) Clean-GT reference}\\[2pt]
\begin{tabular}{c c c c}
\hline
...
\hline
2 & R4A1 & 22.6661 & 0.6126 \\
  & R1A4 & 22.6235 & 0.6110 \\
  & R2A2 & \textbf{22.8628} & \textbf{0.6244} \\
\hline
6 & R4A1 & 24.3446 & 0.7068 \\
  & R1A4 & 24.2957 & 0.7048 \\
  & R2A2 & \textbf{24.6340} & \textbf{0.7216} \\
\hline
10 & R4A1 & 25.1894 & 0.7525 \\
   & R1A4 & 25.1505 & 0.7512 \\
   & R2A2 & \textbf{25.5066} & \textbf{0.7688} \\
\hline
\end{tabular}

\vspace{4pt}
\textbf{(b) Same-noise-GT reference}\\[2pt]
\begin{tabular}{c c c c}
\hline
SNR (dB) & Allocation & PSNR & SSIM \\
\hline
