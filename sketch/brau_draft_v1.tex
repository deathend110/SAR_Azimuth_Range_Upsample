...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{../Exp1_MainResult_Output/Exp1_MainResult_PSNR_curves.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{../Exp1_MainResult_Output/Exp1_MainResult_SSIM_curves.png}
\caption{Main fixed-budget SSIM results under different range-azimuth allocation strategies.}
\label{fig:main_ssim}
\end{figure}

Table~\ref{tab:main_gain} reports the gain of the best bidirectional allocation over the best unidirectional allocation. Across all tested budgets, the best bidirectional group consistently outperforms the best unidirectional group in both PSNR and SSIM. The PSNR improvement ranges from 0.2856 dB to 0.5072 dB, and the SSIM improvement ranges from 0.0169 to 0.0221. All paired Wilcoxon tests are statistically significant.

\begin{table}[t]
\caption{Gain of the best bidirectional allocation over the best unidirectional allocation.}
\label{tab:main_gain}
\centering
...
\hline
4  & R2A2 & R4A1  & 0.2856 & 0.0169 \\
6  & R2A3 & R1A6  & 0.4170 & 0.0208 \\
8  & R2A4 & R8A1  & 0.4576 & 0.0215 \\
9  & R3A3 & R9A1  & 0.4685 & 0.0214 \\
10 & R5A2 & R10A1 & 0.5072 & 0.0221 \\
\hline
\end{tabular}
\end{table}
...
For this analysis, we use $Q=4$ and compare four representative allocations: no upsampling (R1A1), range-only upsampling (R4A1), azimuth-only upsampling (R1A4), and bidirectional upsampling (R2A2). Fig.~\ref{fig:mechanism} visualizes the intermediate spectra. The most informative difference appears after range compression (RC) and is preserved after range cell migration correction (RCMC). Compared with the unidirectional groups, R2A2 produces a cleaner spectral concentration with lower off-support and directional leakage.

\begin{figure*}[t]
\centering
\includegraphics[width=0.95\textwidth]{../Exp2_Mechanism_Output/Exp2_2D_Mechanism_Figure.pdf}
\caption{Two-dimensional spectral evidence for different allocation strategies. Bidirectional allocation shows lower off-support and directional leakage after SAR processing.}
\label{fig:mechanism}
\end{figure*}
...
Table~\ref{tab:mechanism_metrics} quantifies this observation. After RC, R2A2 achieves the lowest off-support ratio, range leakage ratio, and azimuth leakage ratio among all upsampled groups. The same ordering is retained after RCMC. This result supports the interpretation that bidirectional allocation better matches the two-dimensional range-azimuth structure of SAR data after coherent processing. The mechanism evidence is therefore consistent with the final reconstruction metrics: the bidirectional group is not only better in PSNR and SSIM, but also produces cleaner intermediate spectra.
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{../Exp3B_ZT_NCT_RT_Output/Exp3B_GainBars.png}
...
G. Franceschetti, V. Pascazio, and G. Schirinzi, ``Processing of signum coded SAR signal: Theory and experiments,'' \textit{IEE Proceedings F Radar and Signal Processing}, vol. 138, no. 3, pp. 192--198, 1991.

\bibitem{zhao2019onebit}
B. Zhao, Y. Huang, and Z. Bao, ``One-bit SAR imaging based on single-frequency thresholds,'' \textit{IEEE Transactions on Geoscience and Remote Sensing}, vol. 57, no. 9, pp. 7017--7032, 2019.

\bibitem{demir2018onebit}
O. Demir and E. Ercelbi, ``One-bit compressive sensing with time-varying thresholds in synthetic aperture radar imaging,'' \textit{IET Radar, Sonar \& Navigation}, vol. 12, no. 5, pp. 543--550, 2018.

\bibitem{zhao2021lowprecision}
B. Zhao, Y. Huang, and T. Jin, ``Strategy for SAR imaging quality improvement with low-precision sampled data,'' \textit{IEEE Transactions on Geoscience and Remote Sensing}, vol. 59, no. 6, pp. 5005--5020, 2021.

\bibitem{nie2025fixed}
Y. Nie, Y. Huang, B. Zhao, and J. Yang, ``One-bit synthetic aperture radar imaging based on fixed-threshold with slow-time fluctuations,'' \textit{IEEE Transactions on Geoscience and Remote Sensing}, 2025.

\bibitem{si2023dequantization}
Y. Si, Y. Huang, B. Zhao, J. Yang, and T. Jin, ``A convolutional de-quantization network for harmonics suppression in one-bit SAR imaging,'' \textit{IEEE Transactions on Geoscience and Remote Sensing}, 2023.
\end{thebibliography}
