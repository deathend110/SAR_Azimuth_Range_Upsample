...
$\mathrm{R}\sqrt{6}\mathrm{A}\sqrt{6}$ &
26.4889 & 0.8232 & 1.0160 \\
\hline
...
$\mathrm{R}\sqrt{10}\mathrm{A}\sqrt{10}$ &
26.9322 & 0.8423 & \textbf{1.0198} \\
\hline
\end{tabular}
\vspace{2pt}
...
Table~\ref{tab:evaluation_indexes} reports the RT results under the common $A_s=0.6$ setting. R1A1 gives the lowest reconstruction fidelity, whereas a bidirectional group reaches the highest PSNR and SSIM at every listed budget. For $Q_{\mathrm{up}}=3$, $\mathrm{R}\sqrt{3}\mathrm{A}\sqrt{3}$ improves on the best unidirectional allocation by 0.2393~dB and 0.0137 SSIM. At $Q_{\mathrm{up}}=4$, R2A2 obtains 26.0623~dB and 0.8026. At $Q_{\mathrm{up}}=6$, R2A3 and R3A2 retain the highest PSNR and SSIM, respectively. The balanced fractional allocation $\mathrm{R}\sqrt{6}\mathrm{A}\sqrt{6}$ reaches 26.4889~dB and 0.8232, trailing the corresponding maxima by only 0.0697~dB and 0.0014.

Related plateaus appear at $Q_{\mathrm{up}}=8$ and 10. The balanced fractional allocation $\mathrm{R}\sqrt{8}\mathrm{A}\sqrt{8}$ reaches 26.8473~dB and 0.8364 SSIM, only 0.0279~dB above R2A4 and with nearly identical SSIM. R3A3 is best at $Q_{\mathrm{up}}=9$ with 26.9167~dB and 0.8400 SSIM. At $Q_{\mathrm{up}}=10$, $\mathrm{R}\sqrt{10}\mathrm{A}\sqrt{10}$ obtains 26.9322~dB and 0.8423 SSIM. No significant paired difference is detected relative to R2A5 ($p_{\mathrm{PSNR}}=0.6714$ and $p_{\mathrm{SSIM}}=0.7453$), while R5A2 retains the highest means of 26.9702~dB and 0.8427. Across all six budgets, paired comparisons between the mean-best bidirectional and unidirectional groups give PSNR gains of 0.2393--0.5072~dB, with $p_{\mathrm{PSNR}}\leq4.38\times10^{-8}$ and $p_{\mathrm{SSIM}}\leq4.05\times10^{-13}$.

ENL varies less across allocations and is reported as a descriptive speckle statistic rather than a primary ranking criterion. Its maximum coincides with the fidelity-optimal group at $Q_{\mathrm{up}}=4$ and 9, whereas the balanced fractional group has the largest ENL at $Q_{\mathrm{up}}=10$. The consistent PSNR and SSIM results indicate that bidirectionality, rather than exact balance, is the main allocation property under the separable RT.
