...
Table~\ref{tab:evaluation_indexes} reports the RT results under the common $A_s=0.6$ setting. R1A1 gives the lowest reconstruction fidelity, whereas a bidirectional group reaches the highest PSNR and SSIM at every listed budget. For $Q_{\mathrm{up}}=3$, $\mathrm{R}\sqrt{3}\mathrm{A}\sqrt{3}$ improves on the best unidirectional allocation by 0.2393~dB and 0.0137 SSIM. At $Q_{\mathrm{up}}=4$, R2A2 obtains 26.0623~dB and 0.8026. At $Q_{\mathrm{up}}=6$, R2A3 and R3A2 are nearly indistinguishable, with PSNR values of 26.5585 and 26.5525~dB.

The same plateau appears at $Q_{\mathrm{up}}=8$. The fractional balanced allocation $\mathrm{R}\sqrt{8}\mathrm{A}\sqrt{8}$ reaches 26.8473~dB and 0.8364 SSIM, only 0.0279~dB above R2A4 and with nearly identical SSIM. R3A3 is best at $Q_{\mathrm{up}}=9$ with 26.9167~dB and 0.8400 SSIM, while R5A2 is best at $Q_{\mathrm{up}}=10$ with 26.9702~dB and 0.8427 SSIM. Across all six budgets, paired comparisons between the mean-best bidirectional and unidirectional groups give PSNR gains of 0.2393--0.5072~dB, with $p_{\mathrm{PSNR}}\leq4.38\times10^{-8}$ and $p_{\mathrm{SSIM}}\leq4.05\times10^{-13}$.
