...
Synthetic aperture radar (SAR) forms high-resolution images through coherent processing of echoes collected along a synthetic aperture \cite{cumming2005sar,curlander1991sar}. Higher resolution and wider swaths increase the number of complex samples that must be acquired, buffered, processed, and transmitted. This data burden is especially restrictive for spaceborne and airborne platforms, where onboard memory, downlink capacity, and power are limited. Block-adaptive quantization, flexible dynamic quantization, and predictive coding reduce the SAR raw-data rate while preserving information required by the imaging processor \cite{kwok1989baq,attema2010fdbaq,magli2003lossy}. One-bit SAR takes this reduction to the extreme by retaining only the signs of the in-phase and quadrature echo components. This binary representation can greatly reduce storage and transmission requirements, but it also changes the signal entering every subsequent coherent processing stage.

Sign quantization discards the continuous amplitude of the complex echo and generates nonlinear distortion. Unlike image-domain compression applied after focusing, one-bit acquisition acts on the two-dimensional raw echo before range compression and azimuth synthesis. The range and slow-time axes therefore determine both the sampled signal support and the placement of quantization distortion. Oversampling can create additional spectral separation before the data are returned to the original imaging grid, but the benefit depends on which dimension is enlarged. Concentrating all additional samples in range changes only the fast-time grid, while azimuth-only processing changes only the slow-time grid. Because SAR focusing uses information from both axes, the dimensional allocation of a limited sampling budget is itself a signal-processing decision.

Signum-coded echoes were first used for SAR image formation \cite{franceschetti1991signum}. Time-domain convolution was then developed for binary radar signals \cite{alberti1991time}. Dedicated hardware demonstrated real-time one-bit SAR processing \cite{cappuccino1996processor}. Binary signals were also extended to SAR interferometry \cite{fornaro1997interferometry}. A subsequent synthesis clarified one-bit SAR imaging properties \cite{pascazio1998onebit}. Phase-quantization experiments connected binary acquisition with spectral distortion \cite{franceschetti1999phase}.

Compressive sampling framed recovery from incomplete measurements \cite{candes2008intro}. One-bit compressive sensing formulated recovery from signs \cite{boufounos2008onebit}. Fast algorithms addressed consistent one-bit recovery \cite{laska2011trust}. Binary stable embeddings characterized the preserved measurement geometry \cite{jacques2013robust}. A maximum-a-posteriori model adapted one-bit recovery to SAR \cite{dong2015map}. Two-level block sparsity exploited structured radar targets \cite{wang2019enhanced}. Sparse logistic regression provided another one-bit SAR model \cite{ge2023sparse}.

Recent work emphasizes threshold design and quantization-distortion suppression. Time-varying thresholds retain amplitude-related information \cite{demir2018onebit}. Single-frequency thresholds relocate dominant harmonics \cite{zhao2019onebit}. Low-precision sampling can improve SAR image quality \cite{zhao2021lowprecision}. Two-bit acquisition offers a related rate-quality tradeoff \cite{liu2023twobit}. Learned de-quantization suppresses one-bit SAR harmonics \cite{si2023dequantization}. Fixed thresholds with slow-time fluctuations provide another acquisition mechanism \cite{nie2025fixed}. These methods couple sampling with a particular threshold or reconstruction design. They do not compare range-azimuth allocations under a common resource constraint.
...
\begin{itemize}
    ...
    \item Controlled experiments show that bidirectional allocations consistently outperform unidirectional allocations under a common separable random threshold, with a near-optimal plateau among several bidirectional factor pairs.
...
Let $s[n,m]\in\mathbb{C}^{N_r\times N_a}$ denote the sampled complex SAR echo, where $n$ and $m$ index range and azimuth, respectively. For range and azimuth factors $R$ and $A$, define
\begin{equation}
...
\end{equation}
The factor $RA$ preserves the complex-echo scale after spectral embedding. The range sampling frequency becomes $F_s^{\uparrow}=RF_s$, while the azimuth sampling grid is enlarged by $A$. Given a threshold $u[n,m]\in\mathbb{C}^{N_r^{\uparrow}\times N_a^{\uparrow}}$, one-bit quantization is applied separately to both components:
...
\end{equation}
where $\mu_I$ and $\sigma_I^2$ are the mean and variance within a homogeneous ROI. ENL is calculated for 20 samples from the field and port datasets. A $64\times64$ ROI is selected on each GT image and then reused unchanged for every allocation. PSNR and SSIM use all paired samples.

For each budget, the best bidirectional and unidirectional groups are selected by mean PSNR. Paired Wilcoxon signed-rank tests \cite{wilcoxon1945ranking} are applied to the corresponding paired sample vectors.

\section{Experiments and Results}

\subsection{Mechanism Analysis Through Spectral Leakage}

Phase-quantized SAR processing produces nonlinear harmonic distortion \cite{franceschetti1999phase}. Single-frequency threshold analysis shows that threshold spectra can relocate dominant harmonics \cite{zhao2019onebit}. Learned de-quantization methods likewise target harmonic suppression in reconstructed one-bit SAR images \cite{si2023dequantization}. In the two-dimensional SAR data considered here, this distortion appears along both range and azimuth. The working interpretation of BRAU is that the effective SAR echo occupies a two-dimensional range-azimuth spectral support, whereas a unidirectional allocation enlarges only one spectral axis. In contrast, a bidirectional allocation supplies frequency-domain redundancy in both dimensions and can reduce the overlap between quantization distortion and the effective support. Fig.~\ref{fig:mechanism} compares representative allocations for $Q_{\mathrm{up}}=4$ over a common normalized-frequency window of $[-2,2]\times[-2,2]$.
...
\begin{tabular}{c c c c}
\hline
SNR (dB) & Allocation & PSNR & SSIM \\
\hline
$-2$ & R4A1 & 20.2581 & 0.4639 \\
     & R1A4 & 20.2338 & 0.4630 \\
     & R2A2 & \textbf{20.4013} & \textbf{0.4733} \\
\hline
2.5 & R4A1 & 22.8908 & 0.6268 \\
    & R1A4 & 22.8998 & 0.6264 \\
    & R2A2 & \textbf{23.1022} & \textbf{0.6392} \\
\hline
7 & R4A1 & 24.6365 & 0.7221 \\
  & R1A4 & 24.6290 & 0.7210 \\
  & R2A2 & \textbf{24.9158} & \textbf{0.7368} \\
\hline
11.5 & R4A1 & 25.3560 & 0.7619 \\
     & R1A4 & 25.3278 & 0.7609 \\
     & R2A2 & \textbf{25.6860} & \textbf{0.7789} \\
\hline
$\infty$ & R4A1 & 25.7621 & 0.7854 \\
         & R1A4 & 25.7108 & 0.7840 \\
         & R2A2 & \textbf{26.0808} & \textbf{0.8029} \\
\hline
\end{tabular}

\vspace{2pt}
\parbox{0.96\linewidth}{\footnotesize  $\infty$ refers to the noise free casel.}
\end{table}

\subsection{Noisy Ablation}
...
\begin{equation}
    ...
    P_n=P_s10^{-\mathrm{SNR}_{n}/10}.
\end{equation}
Each complex Gaussian realization is then generated, centered, and added according to
\begin{equation}
    \begin{aligned}
    ...
    \end{aligned}
\end{equation}
Before upsampling and one-bit quantization, three seeded Gaussian draws per sample are power-scaled across SNRs, with each resulting noisy echo shared by all allocations. R4A1, R1A4, and R2A2 are evaluated at $Q_{\mathrm{up}}=4$ and $A_s=0.6$; Table~\ref{tab:noise_ablation} reports four representative finite SNRs and the no-added-noise baseline.

R2A2 achieves the highest PSNR and SSIM under every condition in Table~\ref{tab:noise_ablation}, with its PSNR/SSIM increasing from 20.4013~dB/0.4733 at $-2$~dB to 26.0808~dB/0.8029 without added noise.
