...
For the noisy experiment, 50 Gaussian noise realizations generated with different random seeds are added to each sample at SNR levels of $-2$, $2.5$, $7$, and $11.5$~dB, with the noise-free case retained as a baseline. Each resulting noisy echo is shared by all allocations. R4A1, R1A4, and R2A2 are evaluated at $Q_{\mathrm{up}}=4$ and $A_s=0.6$.
...
R2A2 achieves the highest PSNR and SSIM under every condition in Table~\ref{tab:noise_ablation}, with PSNR increasing from $20.4013$ to $26.0808$~dB and SSIM from $0.4733$ to $0.8029$ as the noise level decreases from $-2$~dB to the noise-free case.
