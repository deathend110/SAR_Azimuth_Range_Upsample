...
\end{equation}
Each complex Gaussian realization is centered and added to the raw echo according to
\begin{equation}
    \begin{aligned}
...
\end{equation}
where $\mu_I$ and $\sigma_I^2$ are the mean and variance within a homogeneous ROI. ENL is calculated for 20 samples from the field and port datasets. A $64\times64$ ROI is selected on each GT image and then reused unchanged for every allocation. PSNR and SSIM use all paired samples. For each budget, the best bidirectional and unidirectional groups are selected by mean PSNR. Paired Wilcoxon signed-rank tests \cite{wilcoxon1945ranking} are applied to the corresponding paired sample vectors.

The mechanism experiment compares R1A1, R4A1, R1A4, and R2A2 at $Q_{\mathrm{up}}=4$ after range compression. Their spectra are evaluated over a common normalized-frequency window of $[-2,2]\times[-2,2]$, and the reference support mask uses $\tau=0.35$. The threshold-sensitivity experiment evaluates the best bidirectional allocations at $Q_{\mathrm{up}}=4,6,9$ over the tested $A_s$ values.

For the noisy experiment, three seeded Gaussian realizations per sample are power-scaled to $-2$, 2.5, 7, and 11.5~dB, with the no-added-noise case retained as a baseline. Each resulting noisy echo is shared by all allocations. R4A1, R1A4, and R2A2 are evaluated at $Q_{\mathrm{up}}=4$ and $A_s=0.6$.
...
Fig.~\ref{fig:scene_examples} shows that both unidirectional allocations improve over one-bit imaging without upsampling, while R2A2 more faithfully preserves extended target boundaries and local scattering textures.
...
The noise robustness comparison is reported in Table~\ref{tab:noise_ablation}.
