...
\begin{abstract}
One-bit acquisition reduces the raw-data burden of high-resolution synthetic aperture radar (SAR), but how a fixed pre-quantization upsampling budget should be allocated between range and azimuth remains unclear. We propose bidirectional range-azimuth upsampling (BRAU), which distributes this budget across both echo dimensions rather than concentrating it on one axis. Under a common separable random threshold and five integer budgets, bidirectional allocations consistently outperform unidirectional ones and form a near-optimal plateau. Range-compressed spectra further show reduced off-support and directional leakage, linking the gain to two-dimensional spectral redundancy.
\end{abstract}
