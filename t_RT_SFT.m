...
subplot(131);imagesc(img_gt);axis image;colorbar;title("GT As: "+num2str(As_rt));
...
% 生成RT噪声
[U_master_patch, ~, ~] = Range_Build_RT(signal60_patch_high, As_rt);
...
Range_title = [
    "Range Upsample RT q" + num2str(Range_q) + " 1bit"; 
    "SSIM: " + num2str(ssim(Range_Upsample, img_gt))+"   PSNR: " + num2str(psnr(Range_Upsample, img_gt))
];
subplot(132);imagesc(Range_Upsample);axis image;colorbar;title(Range_title);

%% Range Upsample SingleSFT
% 上采样
signal60_patch_high = range_upsample_fft(signal60_input, Range_q);
...
% 生成SingleSFT噪声
[U_master_patch, ~, ~] = Build_2D_SingleSFT(signal60_patch_high, nrn_up, Fs_up, As_rt);
...
Range_Upsample = normalize_image(roi_crop);
Range_title = [
    "Range Upsample Single SFT q" + num2str(Range_q) + " 1bit"; 
    "SSIM: " + num2str(ssim(Range_Upsample, img_gt))+"   PSNR: " + num2str(psnr(Range_Upsample, img_gt))
    ];
...
% 一次生成距离向全局RT阈值
function [U, sigma, A_rt] = Range_Build_RT(signal_up, As)
...
% 将RT的均匀分布相位换成一个单调递增的时间向量。相当于没有参数调优的SFT
function [U, sigma, A_rt] = Build_2D_SingleSFT(signal_up, nrn_up, Fs_up, As)
    % 上采样后的阈值和没有上采样的阈值是一致的，这里可以统一用sigma
    sigma = sqrt(2 / pi) * mean(abs(signal_up(:)));
    A_rt = As * sigma;
    ...
    phi = 2 * pi * fast_time_rel;     % 序列级唯一随机相位
    U = A_rt * exp(1i * phi);         % [q*1200 x 1]
end
