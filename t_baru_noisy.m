...
% 生成2D RT阈值
[U_master_patch, sigma, A_rt] = Build_2D_SplitRT(signal60_patch_high, As_rt);
...
subplot(223);imagesc(Azimuth_Range_Upsample);axis image;colorbar;title(Azimuth_Range_title);
...
% 上采样
signal60_patch_high_noisy = two_dim_upsample_fft(signal60_input_noisy, Azimuth_q_m, Range_q_m);

% 生成2D RT阈值
[U_master_patch, sigma, A_rt] = Build_2D_SplitRT(signal60_patch_high_noisy, As_rt);
...
IMG_high  = SAR_Imaging(RCMC_crop, S60.lambda, S60.Fs, S60.R0, S60.C, S60.v, S60.tnan, S60.Ta, S60.prf);
roi_crop = abs(IMG_high(S60.nrn/2-S60.R_total/2+1:S60.nrn/2+S60.R_total/2, S60.nan/2-S60.A_num/2:S60.nan/2+S60.A_num/2-1));
Azimuth_Range_Upsample_noisy = normalize_image(roi_crop);
% ssim(Azimuth_Range_Upsample_noisy, img_gt)
...
Azimuth_Range_title = [
    "Azimuth-Range MixUpsample noisy q" + num2str(q) + " 1bit"; 
    "SSIM: " + num2str(ssim(Azimuth_Range_Upsample_noisy, img_gt_noisy))+"   PSNR: " + num2str(psnr(Azimuth_Range_Upsample_noisy, img_gt_noisy))
    ];
subplot(224);imagesc(Azimuth_Range_Upsample_noisy);axis image;colorbar;title(Azimuth_Range_title);
...
function [U, sigma, A_rt] = Build_2D_SplitRT(signal_up_2d, As)
