...
plot_threshold_panel(data.rxax_configs, data.As_list, ...
    data.ssim_mean_nct, data.ssim_std_nct, "NCT", data.total_samples);
plot_threshold_panel(data.rxax_configs, data.As_list, ...
    data.ssim_mean_rt, data.ssim_std_rt, "SplitRT", data.total_samples);

function plot_threshold_panel(rxax_configs, As_list, ssim_mean, ssim_std, threshold_name, total_samples)
    figure("Color", "w", "Position", [100, 100, 900, 500], "Name", threshold_name);
    ...
    for cfg_idx = 1:numel(rxax_configs)
        ...
        curve_handles(cfg_idx) = errorbar(As_list, y, e, "-o", ...
    ...
    title(sprintf("%s: SSIM-A_s curves (mean \\pm std, n=%d)", ...
        threshold_name, total_samples), "FontSize", 13);
